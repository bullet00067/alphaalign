...
import { RebalanceResult } from '../types/rebalance'

interface SimulationReportProps {
  reportData: RebalanceResult | null
}

export default function SimulationReport({ reportData }: SimulationReportProps) {
    ...
    <div className="mt-8 bg-slate-800/80 backdrop-blur-xl border border-blue-500/30 p-6 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
          ...
          <p
            className={`text-xl font-bold ${
              reportData.total_unrealized_pnl >= 0 ? 'text-green-400' : 'text-red-400'
            }`}
          >
            {reportData.total_unrealized_pnl >= 0 ? '+' : ''}
            {reportData.total_unrealized_pnl.toLocaleString()}
        ...
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
          ...
          <p className="text-xl font-bold text-orange-400">
            ${reportData.estimated_total_transaction_cost.toLocaleString()}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {reportData.reports.map((report, idx) => (
          <div
            key={idx}
            className="bg-slate-900/30 rounded-xl p-5 border border-slate-700/50"
          >
              ...
              <div>
                <h3 className="font-semibold text-lg flex items-center gap-2">
                  ...
                  {report.unrealized_pnl !== 0 && (
                    ...
                    >
                      {report.unrealized_pnl > 0 ? '+' : ''}
                      {report.unrealized_pnl.toLocaleString()}
                    </span>
                  )}
                </h3>
                <p className="text-sm text-slate-400">
                  實際比例:{' '}
                  <span className="font-semibold text-slate-300">{report.current_pct || '0%'}</span>{' '}
                  <ArrowRight className="inline w-3 h-3 mx-1 text-slate-500" /> 目標比例:{' '}
                  <span className="font-semibold text-blue-400">{report.target_pct}</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm text-slate-400">
                  現值 ({report.current_pct || '0%'}){' '}
                  <ArrowRight className="inline w-3 h-3 mx-1" /> 目標 ({report.target_pct})
                </p>
                <p className="font-mono">
                  ${report.current_value.toLocaleString()}{' '}
                  <ArrowRight className="inline w-3 h-3 mx-1 text-slate-600" />{' '}
                  ${report.target_value.toLocaleString()}
                </p>
              </div>
            </div>
            ...
            {report.actions && report.actions.length > 0 ? (
              <div className="space-y-2 mt-4 pt-4 border-t border-slate-700/50">
                {report.actions.map((action, aidx) => (
                  <div
                    key={aidx}
                    className="flex justify-between items-center bg-slate-800/50 p-3 rounded-lg text-sm font-mono"
                  >
