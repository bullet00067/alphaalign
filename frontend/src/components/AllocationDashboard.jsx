...
import { Plus, AlertCircle, CheckCircle2, TrendingUp, Save, Loader2, Sparkles, Trash2, Edit2, Wallet, Zap, Shield } from 'lucide-react';
...
台幣活存 50000`}
        ...
        {/* Rebalancing Strategy Toggle Card */}
        <div className="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl shadow-xl transition-all duration-300">
          <div className="flex items-center gap-2 mb-4">
            <div className="p-1.5 bg-blue-500/20 rounded-lg text-blue-400">
              {momentumMode ? <Zap size={18} className="animate-pulse" /> : <Shield size={18} />}
            </div>
            <h3 className="text-base font-semibold text-white">再平衡交易策略 (Rebalancing Strategy)</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Risk Diversification Mode */}
            <div
              onClick={() => setMomentumMode(false)}
            ...
            >
              <div>
                <div className="flex items-center gap-2.5 mb-2">
                  <div className={`p-2 rounded-lg transition-colors ${!momentumMode ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-400'}`}>
                    <Shield size={18} />
                  </div>
                  <span className="font-semibold text-sm text-slate-100">風險分散模式 (Diversified)</span>
                </div>
              ...
              <div className="mt-3 flex items-center justify-between border-t border-slate-700/30 pt-2 text-[10px]">
                ...
                <span className={`px-2 py-0.5 rounded-md font-medium ${!momentumMode ? 'bg-blue-500/10 text-blue-400' : 'bg-slate-800/80 text-slate-500'}`}>
            ...
            >
              <div>
                <div className="flex items-center gap-2.5 mb-2">
                  <div className={`p-2 rounded-lg transition-colors ${momentumMode ? 'bg-indigo-500/20 text-indigo-400' : 'bg-slate-800 text-slate-400'}`}>
                    <Zap size={18} />
                  </div>
                  <span className="font-semibold text-sm text-slate-100 flex items-center gap-1.5">
                    強勢股加碼模式 (Momentum) 
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 border border-amber-500/30 font-medium">強勢</span>
                  </span>
                </div>
                <p className="text-xs text-slate-400 leading-relaxed">
              ...
              <div className="mt-3 flex items-center justify-between border-t border-slate-700/30 pt-2 text-[10px]">
                <span className="text-slate-500">賣出規則：維持股數最多優先</span>
                <span className={`px-2 py-0.5 rounded-md font-medium ${momentumMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-slate-800/80 text-slate-500'}`}>
                  {momentumMode ? '啟用中' : '可選用'}
                </span>
              </div>
            </div>
