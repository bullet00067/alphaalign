...
import { HistoryRecord } from '../types/rebalance'
...
export default function HistoryView({ historyData, onRestore, onDelete }: HistoryViewProps) {
    ...
    return (
      <div className="bg-slate-800/20 border border-slate-700/50 rounded-2xl p-8 text-center text-slate-500">
        ...
        <p className="text-xs mt-1">
          點擊下方的「更新配置並產出報告 (Save & Rebalance)」即可建立第一筆快照！
        </p>
      </div>
    )
  }
    ...
    <div className="bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl shadow-xl space-y-4">
      ...
      <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
        {historyData.map(item => (
          <div
            key={item.id}
            className="bg-slate-900/50 border border-slate-700/30 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-slate-600 transition-colors"
          >
                ...
                <div>
                  ...
                  <p
                    className={`text-sm font-semibold ${
                      item.unrealized_pnl >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}
                  >
                    {item.unrealized_pnl >= 0 ? '+' : ''}
                    {item.unrealized_pnl.toLocaleString()} ({item.roi_pct}%)
                  </p>
                </div>
              </div>
            </div>
